...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">Auxiliar - Municípios</h5>
    </div>
    ...
    <div class="card border-top border-top-width-1 border-bottom border-bottom-width-1 rounded-0" style="margin: 10px">
        ...
        <form class="needs-validation" method='POST' action="{{ @$data_aux ? route('aux.municipio.update', $data_aux->id) : route('aux.municipio.store') }}" enctype='multipart/form-data' novalidate>
            @csrf
            @if (@$data_aux)
                @method('patch')
            @endif
            <div class="card" style="padding-top: 20px">            
                <div class="row mb-3">
                    ...
                    <div class="col-lg-8">
                        <input type="text" name="nome" class="form-control" placeholder="Município" value="{{ (@$data_aux->nome) ? $data_aux->nome : '' }}" required>
                            ...
                            <div class="d-inline-flex">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end">
                                        <a href="{{ route('aux.municipio.edit', $item->id) }}" class="dropdown-item">
                                            <i class="ph-note-pencil me-2"></i>
                                            Editar Município
                                        </a>
                                        <form id="delete_form_{{$item->id}}" method="POST" action="{{ route('aux.municipio.destroy', $item->id) }}">
                                            @csrf
                                            @method('delete')
                                            ...
                                            <button type="button" class="dropdown-item" onclick="deleteForm({{ $item->id }})">
                                                ...
                                                <div style="padding-left: 10px" class="text-danger">Excluir Município</div> 
                                            </button>
                                        </form>
                                    </div>
                                </div>
